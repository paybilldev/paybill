import {
  ...
  createLogger,
  loggers,
  createFileLogger, JSONTransport, type LoggerInfo,
  type ILogger,
  EmptyTransport,
} from '../src';
...
describe('/test/index.test.ts', () => {
  ...
  it('should test create logger with file transport', async () => {
    ...
    expect(fileExists(join(logsDir, 'paybill-core.log'))).toBeTruthy();
    expect(fileExists(join(logsDir, 'common-error.log'))).toBeTruthy();

    // test logger file include content
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world1')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world2')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world3')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world4')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world5')
    ).toBeTruthy();

    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world6')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world7')
    ).toBeTruthy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world8')
    ).toBeTruthy();

    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world9')
    ).toBeFalsy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world10')
    ).toBeFalsy();
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), 'hello world11')
    ).toBeFalsy();
    ...
    // test default eol
    expect(
      includeContent(join(logsDir, 'paybill-core.log'), os.EOL)
    ).toBeTruthy();
    expect(
  ...
  it('should use write method to file', async () => {
    ...
    expect(
      matchContentTimes(
        join(logsDir, 'paybill-core.log'),
        process.pid.toString()
      )
    ).toEqual(0);
    expect(
      matchContentTimes(join(logsDir, 'paybill-core.log'), 'hello world')
    ).toEqual(2);
    await removeFileOrDir(logsDir);
  });
  ...
  it('should custom transport', async () => {
    ...
    expect(
      matchContentTimes(join(logsDir, 'paybill-core.log'), 'hello world info')
    ).toEqual(1);
    expect(
      matchContentTimes(join(logsDir, 'paybill-core.log'), 'hello world warn')
    ).toEqual(1);
    expect(
      matchContentTimes(
        join(logsDir, 'paybill-core.log'),
  ...
  describe('Logger getChild method', () => {
    ...
    it('create add category and format in parent', () => {
      const formatFn = vi.fn((info: LoggerInfo<{
        category: string;
      }>) => {
