...
export class UpdateGuard {
  ...
  static fromOptions(model, options) {
    ...
    guard.setAction(_.get(options, 'action', 'update'));
  ...
  sanitize(values: UpdateValues) {
    ...
    const associationsValues = _.pick(values, Object.keys(associations));
    ...
    Object.keys(associationsValues).forEach((association) => {
      ...
      const filterAssociationToBeUpdate = (value) => {
        ...
        if (value[associationKeyName]) {
          return _.pick(value, [associationKeyName, ...Object.keys(associationObj.target.associations)]);
        }
      ...
      const sanitizeValue = (value) => {
        ...
        ['whiteList', 'blackList', 'associationKeysToBeUpdate'].forEach((optionKey) => {
          associationUpdateGuard[`set${_.upperFirst(optionKey)}`](listOfAssociation(this[optionKey], association));
        });
