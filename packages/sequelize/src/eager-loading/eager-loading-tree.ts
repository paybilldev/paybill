...
import { processIncludes } from '../utils/general';
...
const pushAttribute = (node, attribute) => {
  if (_.isArray(node.attributes) && !node.attributes.includes(attribute)) {
    node.attributes.push(attribute);
  }
};

const EagerLoadingNodeProto = {
  afterBuild(db: Sequelize) {
    ...
    if (collection && collection.isParent()) {
      ...
      OptionsParser.appendInheritInspectAttribute(
        _.isArray(this.attributes) ? this.attributes : this.attributes.include,
        collection,
      );
...
export class EagerLoadingTree {
  ...
  static buildFromSequelizeOptions(options: {
    ...
    const root = buildNode({
      model,
      association: null,
      rawAttributes: _.cloneDeep(rootAttributes),
      attributes: _.cloneDeep(rootAttributes),
      order: options.rootOrder,
      children: [],
    });

    const traverseIncludeOption = (includeOption, eagerLoadingTreeParent) => {
      const includeOptions = _.castArray(includeOption);
      ...
      for (const include of includeOptions) {
        ...
        const association = _.isString(include.association)
        ...
        const child = buildNode({
          ...
          association,
          rawAttributes: _.cloneDeep(include.attributes),
          attributes: _.cloneDeep(include.attributes),
  ...
  async load(transaction?: Transaction) {
    ...
    const loadRecursive = async (node, ids = []) => {
      ...
      if (!node.parent) {
        ...
        const otherFindOptions = _.pick(node.includeOption, ['sort']) || {};

        const collection = this.db.modelCollection.get(node.model);

        if (collection && !_.isEmpty(otherFindOptions)) {
        ...
        if (associationType === 'BelongsToArray') {
          ...
          let where: any = { [targetKey]: Array.from(new Set(_.flatten(targetKeyValues))) };
    ...
    const setInstanceAttributes = (node) => {
      ...
      if (!_.isArray(nodeRawAttributes)) {
        return;
      }
      ...
      for (const instance of node.instances) {
        instance.dataValues = _.pick(instance.dataValues, includeAttributes);
      }
    };
