import { FindAttributeOptions, ModelStatic, Op, Sequelize as MainSequelize } from 'sequelize';
...
export class OptionsParser {
  ...
  static appendInheritInspectAttribute(include, collection): any {
    ...
    include.push([
      MainSequelize.literal(`(select relname from pg_class where pg_class.oid = "${collection.name}".tableoid)`),
      '__tableName',
    ]);

    include.push([
      MainSequelize.literal(`
        (SELECT n.nspname
        FROM pg_class c
  ...
  filterByTkToWhereOption() {
    ...
    if (_.isPlainObject(this.options.filterByTk)) {
      const where = {};
  ...
  toSequelizeParams(options: { parseSort?: boolean } = { parseSort: true }) {
    ...
    if (this.options?.include) {
      ...
      queryParams.include.push(..._.castArray(this.options.include));
    }
  ...
  protected parseSort(filterParams) {
    ...
    if (defaultSortField && !this.options?.group) {
      defaultSortField = _.castArray(defaultSortField);
    ...
    for (const sortKey of sort) {
      ...
      if (this.database.isMySQLCompatibleDialect()) {
        ...
        if (this.model.fieldRawAttributesMap[fieldName]) {
          orderParams.push([MainSequelize.fn('ISNULL', MainSequelize.col(`${this.model.name}.${sortField[0]}`))]);
        }
      }
  ...
  protected parseAppends(appends: Appends, filterParams: any) {
    ...
    const sortedAppends = _.sortBy(appendList, (append) => append.split('.').length);
    ...
    const setInclude = (model: ModelStatic<any>, queryParams: any, append: string, parentAs?: string) => {
      ...
      if (
        lastLevel &&
        existIncludeIndex != -1 &&
        _.get(queryParams, ['include', existIncludeIndex, 'attributes', 'include'])?.length == 0
      ) {
