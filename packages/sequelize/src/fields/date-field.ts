...
export class DateField extends Field {
  ...
  bind() {
    ...
    if (this.options.interface === 'created_at') {
      const { model } = this.context.collection;
      // @ts-ignore
      model._timestampAttributes.created_at = this.name;
      // @ts-ignore
      model.refreshAttributes();
    }

    if (this.options.interface === 'updated_at') {
      const { model } = this.context.collection;
      // @ts-ignore
      model._timestampAttributes.updated_at = this.name;
      // @ts-ignore
      model.refreshAttributes();
    }
