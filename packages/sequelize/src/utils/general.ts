...
function patchShowConstraintsQuery(queryGenerator, db) {
  queryGenerator.showConstraintsQuery = (tableName, constraintName) => {
    const lines = [
      ...
      `WHERE table_name='${_.isPlainObject(tableName) ? tableName.tableName : tableName}'`,
    ];
    ...
    if (_.isPlainObject(tableName) && tableName.schema) {
...
export function patchSequelizeQueryInterface(db: Sequelize) {
