...
import { checkIdentifier, md5, snakeCase } from './utils/general';
...
function EnsureAtomicity(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = function (...args: any[]) {
    ...
    try {
      ...
      const createdAssociationKeys = _.difference(afterAssociationKeys, beforeAssociationKeys);
      ...
      const createdRawAttributes = _.difference(afterRawAttributes, beforeRawAttributes);
...
> extends EventEmitter {
  ...
  modelInit() {
    ...
    Object.defineProperty(this.model, 'primaryKeyAttributes', {
      get: function () {
        ...
        if (collection.options.filterTargetKey) {
          const fields = _.castArray(collection.options.filterTargetKey);
    ...
    this.model.options.modelName = this.options.model ? this.options.model.toString() : this.options.name;
  ...
  setField(name: string, options: FieldOptions): Field {
    ...
    database.logger.info(`beforeSetField: ${safeJsonStringify(options)}`, {
    ...
    if (options.source) {
      ...
      if (!sourceCollection) {
        ...
        if (!sourceField) {
          ...
          options = { ..._.omit(sourceField.options, ['name', 'primaryKey']), ...options };
        }
      }
    }
  ...
  async removeFieldFromDb(name: string, options?: QueryInterfaceOptions) {
    ...
    if (this.model.options.timestamps !== false) {
      let timestampsFields = ['created_at', 'updated_at', 'deleted_at'];
  ...
  addIndex(
    ...
    for (const item of indexes) {
      if (_.isEqual(this.normalizeFieldName(item.fields), this.normalizeFieldName(indexName))) {
        return;
      }
