...
import { patchSequelizeQueryInterface, snakeCase } from './utils/general';
import { BaseValueParser, registerFieldValueParsers } from './value-parsers';
import { ViewCollection } from './view-collection';
import { BaseDialect } from './dialects/base-dialect';
import { applyMixins, AsyncEmitter } from './utils/AsyncEmitter';
...
export interface IOptions extends SequelizeOptions {
  tablePrefix?: string;
  migrator?: any;
...
export class Sequelize extends EventEmitter implements AsyncEmitter {
  ...
  constructor(options: Options) {
    ...
    const dialectClass = Sequelize.getDialect(options.dialect);
    ...
    this.logger.debug(
      `create database instance: ${safeJsonStringify(
        ...
        _.omit(this.options, ['storage', 'host', 'password']),
      )}`,
      {
    ...
    try {
      this.sequelize = new MainSequelize(sequelizeOptions);
    } catch (error) {
  ...
  createMigrator({ migrations }) {
    ...
    return new Umzug({
      logger: migratorOptions.logger || console,
      migrations: Array.isArray(migrations) ? _.sortBy(migrations, (m) => m.name) : migrations,
      context,
      storage: new SequelizeStorage({
        tableName: `${this.options.tablePrefix || ''}migrations`,
        modelName: 'migrations',
   ...
   * @internal
   */
  sequelizeOptions(options: Options) {
  ...
  initListener() {
    ...
    this.on('afterDefine', (model) => {
      if (_.get(this.options, 'usingBigIntForId', true)) {
  ...
  collection<Attributes = any, CreateAttributes = Attributes>(
    ...
    this.logger.info(`beforeDefineCollection: ${safeJsonStringify(options)}`, {
      databaseInstanceId: this.instanceId,
    });
  ...
  isSqliteMemory() {
    return this.sequelize.getDialect() === 'sqlite' && _.get(this.options, 'storage') == ':memory:';
  }
  ...
  extendCollection(collectionOptions: CollectionOptions, mergeOptions?: MergeOptions) {
    collectionOptions = _.cloneDeep(collectionOptions);
  ...
  private registerCollectionType() {
    this.collectionFactory.registerCollectionType(InheritedCollection, {
      condition: (options) => {
        return options.inherits && _.castArray(options.inherits).length > 0;
      },
    });
...
applyMixins(Sequelize, [AsyncEmitter]);
registerDialects();
