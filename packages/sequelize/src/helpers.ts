...
export async function parseDatabaseOptionsFromEnv(): Promise<Options> {
...
export async function checkDatabaseVersion(db: Sequelize) {
...
export function registerDialects() {
  [SqliteDialect, MysqlDialect, MariadbDialect, PostgresDialect].forEach((dialect) => {
    Sequelize.registerDialect(dialect);
  });
}
