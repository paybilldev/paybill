...
export class ArrayFieldRepository {
  ...
  async set(
    ...
    instance.set(this.fieldName, _.castArray(options.values));
  ...
  async toggle(
    ...
    const newValue = oldValue.includes(options.value)
      ? _.without(oldValue, options.value)
  ...
  async add(
    ...
    const newValue = [...oldValue, ..._.castArray(options.values)];
    instance.set(this.fieldName, newValue);
    await instance.save({ transaction });
  ...
  async remove(
    ...
    const oldValue = instance.get(this.fieldName) || [];
    instance.set(this.fieldName, _.without(oldValue, ..._.castArray(options.values)));
    await instance.save({ transaction });
