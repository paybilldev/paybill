...
describe('view inference', function () {
  ...
  it('should infer collection fields', async () => {
    ...
    const created_at = UserCollection.model.rawAttributes['created_at'].field;
    expect(inferredFields[created_at]['field']).toBeDefined();

    if (db.isMySQLCompatibleDialect()) {
      expect(inferredFields[created_at]['type']).toBe('datetimeNoTz');
    } else {
      expect(inferredFields[created_at]['type']).toBe('datetimeTz');
    }
