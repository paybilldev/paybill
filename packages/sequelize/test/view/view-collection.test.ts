import { Sequelize, ViewCollection, mockDatabase } from '../../src';
...
DECLARE
  new_group_id BIGINT;
BEGIN
  -- Insert a new group row and get the ID of the newly inserted row.
  INSERT INTO ${GroupCollection.quotedTableName()} (name) VALUES (NEW.group_name) RETURNING id INTO new_group_id;

  -- Insert a new users row, using the ID of the newly inserted groups row as the group_id.
