...
const mockJWTKeys: JwtKeysDecoder = {
  key1: {
    privateKey: { kty: 'RSA', kid: 'key1', alg: 'RS256' },
    publicKey: { kty: 'RSA', kid: 'key1', alg: 'RS256' },
  },
  key2: {
...
async function buildTestApp(customConfig?: JWTConfiguration) {
  ...
  await app.register(jwtPlugin, { config: customConfig });
  await app.ready();
  return app;
}
...
describe('Well-Known Endpoints (Supertest) with JWT override', () => {
  it('returns only public non-HMAC keys from JWKS', async () => {
    ...
    const res = await supertest(app.server).get(`${prefix}/.well-known/jwks.json`);
  ...
  it('returns valid OpenID configuration with registration endpoint', async () => {
    ...
    const res = await supertest(app.server).get(`${prefix}/.well-known/openid-configuration`);
    expect(res.status).toBe(200);
    expect(res.body.issuer).toBe(jwtConfigMock.issuer);
    expect(res.body.registration_endpoint).toBe('http://localhost/oauth/clients/register');
